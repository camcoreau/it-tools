  ...
  <div class="home-page">
    <section class="hero-grid">
      <div class="hero-main surface">
        ...
        <p>
          Search, convert, inspect, generate and troubleshoot with a trusted toolkit that runs
          directly in your browser. Built for everyday CamCore administration and support work.
        </p>

        <div class="hero-actions">
          <a class="hero-button primary" href="#utility-directory">Browse all utilities</a>
          <button class="hero-button" type="button" @click="$el?.closest('.home-page')?.previousElementSibling?.querySelector('input')?.focus?.()">
            Search tools
          </button>
        </div>
    ...
    <section v-if="newTools.length > 0" class="tool-section featured-section surface">
      <div class="section-head compact">
        <div>
          ...
          <h2>New utilities</h2>
          <p>The newest tools available in the CamCore collection.</p>
        </div>
    ...
    <section id="utility-directory" class="tool-section directory-section">
      <div class="section-head directory-heading">
        <div>
          <span class="section-kicker">Complete directory</span>
          <h2>All utilities</h2>
          <p>Choose a category below or use the search field in the header to jump directly to a tool.</p>
        </div>
      ...
      <div class="tool-grid directory-grid">
    ...
    <section class="privacy-panel surface" aria-label="IT Tools privacy and support">
      <div>
        <span class="section-kicker">Private by design</span>
        <h2>Your work stays with your device.</h2>
        <p>CamCore IT Tools is delivered from the private network. Most utilities complete their work locally in the browser rather than sending your input to an external service.</p>
      </div>
...
.hero-glow {
  ...
  bottom: -315px;
  border-radius: 50%;
  background: radial-gradient(circle, rgba(84, 186, 255, 0.27), transparent 70%);
  pointer-events: none;
}
...
.hero-button.primary {
  border-color: rgba(84, 186, 255, 0.56);
  background: linear-gradient(135deg, rgba(84, 186, 255, 0.29), rgba(143, 115, 255, 0.22));
}
...
.trust-row i {
  ...
  border-radius: 50%;
  background: var(--blue);
  box-shadow: 0 0 11px rgba(84, 186, 255, 0.75);
}
